# Migrating Expo UI to EduDS — Educational Design System

Read `artifacts/edu-ds/docs/AGENTS.md` and
`artifacts/edu-ds/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/edu-ds/lib/native-theme` and
  `@workspace/edu-ds/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/edu-ds/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/edu-ds` and retains only
