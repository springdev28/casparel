...
- ~~Consider an institution tier only once a school has actually asked.~~ **Superseded** — the owner asked; `institutional` shipped 15 August as a sales-led per-seat licence (see the latest revision note). The Phase 5 work that remains for it: validate where each quote belongs inside the public US\$2.50–US\$3.00 range against measured per-seat cost and support needs, and decide whether large licences deserve volume steps.
