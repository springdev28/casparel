...
- ~~Consider an institution tier only once a school has actually asked.~~ **Superseded** — the owner asked; `institutional` shipped 15 August as a sales-led per-seat licence (see the latest revision note). The Phase 5 work that remains for it: validate \$1.50/seat against measured per-seat cost once the cost basis exists, and decide whether large licences deserve volume steps.
