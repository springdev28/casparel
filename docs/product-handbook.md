...
- Capacity limits are enforced on the routes that create classes, rosters, activities, lists, goals and canvases. Rows created by other surfaces (forum posts, messages, list items, goal path steps — including resources attached to a goal — schedule blocks, study sessions, learning evidence including adaptive-dashboard check-ins, and canvas objects) are not yet capped.
