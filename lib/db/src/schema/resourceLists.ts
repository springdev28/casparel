...
export const resourceListsTable = pgTable(
  "resource_lists",
  {
    ...
    // Null means private. Tokens are random capabilities, not enumerable ids;
    // revoking a share clears this value and invalidates the old URL.
  ...
  (table) => [
    uniqueIndex("resource_lists_share_token_idx").on(table.shareToken),
  ],
);
...
export const listItemsTable = pgTable(
  ...
  (table) => [
    // Retried or concurrent saves of the same resource must resolve to one
    // durable list item. The API relies on this database invariant for races.
    uniqueIndex("list_items_list_resource_idx").on(table.listId, table.resourceId),
  ],
);
